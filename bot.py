...
def start_add(message):
    if db.check_tutor(message.from_user.id):
        key_ds = random_tutor_code()
        with db.con:
            db.cur.execute(
                f"""INSERT INTO disciplines (tutor, key) VALUES ('{message.from_user.id}','{key_ds}')""")
            ds_id = db.cur.execute(f"""SELECT id FROM disciplines WHERE key = '{key_ds}'""").fetchone()
        ds_id = ds_id[0]
        msg = bot.send_message(message.chat.id, 'Регистрация дисциплины в базе данных. Введите название:')
        bot.register_next_step_handler(msg, dsname, ds_id)
    else:
        bot.send_message(message.chat.id, 'Нет доступа')

def dsname(message, ds_id):
    ds_nm = message.text
    with db.con:
        db.cur.execute(f"""UPDATE disciplines SET discipline_name = '{ds_nm}' WHERE id = '{ds_id}'""")
...
def dsgroup(message, ds_id):
    ...
    msg = bot.send_message(message.chat.id, 'Введите вид аттестации (КП или КР):')
...
def my_disp(message):
    if db.check_tutor(message.from_user.id) and db.check_tutor_disciplines(message.from_user.id):
        ...
        for i in range(len(disciplines)):
            ds_final.append(f'{ds_name[i]} ({ds_gp[i]})')



        disciplines_dict = dict(zip(ds_id, ds_final))
        kb = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
        name_to_kb = []
        for i in range(len(ds_name)):
            name_to_kb.append(f'{ds_name[i]} ({ds_gp[i]})')
    ...
    elif not db.check_tutor(message.from_user.id):
        bot.send_message(message.chat.id, 'Нет доступа')
    else:
        bot.send_message(message.chat.id, 'Дисциплин нет!')
...
def status_look(message, disciplines_dict):
    #try:
        ...
        if bool(len(student_to_check_dict)) == True:
    ...
    #except:
        #bot.send_message(message.chat.id, 'Нет проверки!')
...
def start_study(message):
    if db.check_student(message.from_user.id):
        ...
        if db.check_disp_student(student_group):
            ...
            disciplines_stgr = db.select_info(data = 'group_study', table='students', column='user_id', value=message.from_user.id)
            ds_name = []
            ds_id = []
            for i in disciplines:
                ds_name.append(i[0])
...
def some_action(message, disciplines_dict):
    ...
    if not db.check_get_disp_to_check(message.from_user.id, ds_id):
        kb_1= types.InlineKeyboardButton(text=f"Сдать на проверку", callback_data='sendcheck' + str(ds_id) + " " + str(message.from_user.id))
        kb.add(kb_1)
    else:

        kb_1 = types.InlineKeyboardButton(text="Статус", callback_data='status' + str(ds_id) + " " + str(message.from_user.id))
        kb_2 = types.InlineKeyboardButton(text="Забрать",
                                          callback_data='getback' + str(ds_id) + " " + str(message.from_user.id))
        kb.add(kb_1, kb_2)
...
def send_check(call):
    if call.data.startswith('sendcheck'):
        ...
        bot.send_message(call.message.chat.id, f'ОК')
    ...
    elif call.data.startswith('getback'):
        callback = call.data.split()
        id = callback[0].replace('getback', '')
        db.delete_to_check(callback[1], id)
        bot.send_message(call.message.chat.id, 'Вы забрали работу')
